...
class fMatrix {
    ...
    // friend fMatrix Outer(const fVector &, const fVector &); // Computes the
    // outer product of two vectors.
    friend fMatrix Identity(
        int nSize);  // Returns an nSizexnSize identity matrix.

    friend fMatrix Diag(const fVector &);  // Returns the square matrix
    // with the elements of the vector d along its diagonal. friend fVector
    // Diag(const fMatrix &);	  // Returns the vector consisting of the
    // diagonal elements of the matrix M
    // friend fMatrix Diag(Float, Float,
    //                     Float);  // Returns the 3 x 3 diagonal matrix with x,
    //                     y,
    //                              // and z as its diagonal elements.

    friend double Determinant(
        const fMatrix &);  // Computes the determinant of a square matrix
    friend double Trace(
        const fMatrix &);  // Computes the trace of a square matrix
    friend double OneNorm(
        const fMatrix &);  // Computes the L1-norm of the matrix A, which is the
                           // maximum absolute column sum.
    friend double InfNorm(
        const fMatrix &);  // Computes the Inf-norm of the matrix A, which is
                           // the maximum absolute row sum.

    friend fMatrix Inverse(
        const fMatrix &);  // Computes the inverse of a square matrix.
    friend fMatrix Cholesky(
        const fMatrix
            &);  // Computes Cholesky decomposition of a square matrix.

    friend fVector Mean(
        const fMatrix &);  // Computes column mean value of a matrix.
    friend fMatrix Cov(
        const fMatrix &);  // Returns a covariance matrix of a square matrix.
    friend fMatrix Cov(const fVector &);  // Returns a covariance matrix of a
